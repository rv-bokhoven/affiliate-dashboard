...
export default function OffersManager({ initialOffers, networks, campaignId, campaignName }: OffersManagerProps) {
  ...
  const [currency, setCurrency] = useState('USD'); // <--- NIEUW
  ...
  const handleEditClick = (offer: Offer) => {
    ...
    setCurrency(offer.currency || 'USD'); // <--- Zet valuta goed
  ...
  const handleSave = async () => {
    ...
    const payload = { 
        ...
        capLeads: capLeads ? parseInt(capLeads) : null, 
    ...
    } catch (e) { toast.error('Er ging iets mis'); }
  };
  ...
  const selectClass = "bg-neutral-950 border border-neutral-800 rounded p-2 text-sm text-neutral-200 focus:border-neutral-600 focus:outline-none";
        ...
        <div className={`border border-neutral-800 rounded-xl p-6 mb-8 transition-colors ${editingId ? 'bg-blue-900/10 border-blue-800/30' : 'bg-neutral-900/50'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select className={selectClass + " w-full"} value={networkId} onChange={e => setNetworkId(e.target.value)}>
                ...
                <div>
                    ...
                    <select className={selectClass + " w-full"} value={status} onChange={e => setStatus(e.target.value)}>
                ...
                {/* VALUTA SELECTIE (NIEUW) */}
                <div>
                    <label className="text-xs text-neutral-500 font-medium uppercase mb-1.5 block">Valuta</label>
                    <select className={selectClass + " w-full"} value={currency} onChange={e => setCurrency(e.target.value)}>
                ...
                <div>
                    ...
                    <div className="relative">
                        <span className="absolute left-3 top-2 text-neutral-600 text-xs">{currency === 'EUR' ? '€' : '$'}</span>
                        <input type="number" step="0.01" className={`${inputClass} pl-6`} value={payoutLead} onChange={e => setPayoutLead(e.target.value)} placeholder="0.00" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        <span className="absolute left-3 top-2 text-neutral-600 text-xs">{currency === 'EUR' ? '€' : '$'}</span>
                        <input type="number" step="0.01" className={`${inputClass} pl-6`} value={payoutSale} onChange={e => setPayoutSale(e.target.value)} placeholder="0.00" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        <span className="absolute left-3 top-2 text-yellow-700/50 text-xs">{currency === 'EUR' ? '€' : '$'}</span>
                        <input type="number" step="0.01" className={`${inputClass} pl-6 focus:border-yellow-600/30`} value={capRevenue} onChange={e => setCapRevenue(e.target.value)} placeholder="Unlimited" />
                    </div>
                </div>
            </div>
                        ...
                        offers.map(offer => {
                            ...
                            return (
                                <tr key={offer.id} className="hover:bg-neutral-800/50 transition-colors group">
                                    ...
                                    <td className="p-4 text-right">{sym}{offer.payoutLead} / {sym}{offer.payoutSale}</td>
                                    <td className="p-4 text-right text-xs">
                                        {offer.capLeads ? <div>L: {offer.capLeads}</div> : null}
                                        {offer.capRevenue ? <div>{sym}: {offer.capRevenue}</div> : null}
