    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            className="w-full bg-neutral-950 border border-neutral-800 rounded-md px-3 py-2 text-sm text-white focus:border-neutral-600 outline-none appearance-none"
                        >
                            <option value="">-- Algemeen Project --</option>
                            {offers.map(offer => (
        ...
        <div className="md:col-span-2 space-y-3">
            {loading ? <div className="text-neutral-500 text-center py-10">Laden...</div> : 
             items.length === 0 ? <div className="text-neutral-500 text-center py-10 border border-neutral-800/50 rounded-xl border-dashed">Geen items.</div> : 
             (
                items.map(item => (
                    <div key={item.id} className="flex items-center justify-between p-4 bg-neutral-900/50 border border-neutral-800 rounded-lg group hover:border-neutral-700 transition">
                        <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${item.amount >= 0 ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                                {item.amount >= 0 ? <TrendingUp size={18} /> : <TrendingDown size={18} />}
                            </div>
                            <div>
                                <div className="flex items-center gap-2">
                                    <p className="font-medium text-neutral-200">{item.description}</p>
                                    {/* TOON OFFER TAG ALS DIE ER IS */}
                                    {item.offer && (
                                        <span className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded bg-neutral-800 text-neutral-400 border border-neutral-700">
                                            <Tag size={10} /> {item.offer.name}
                                        </span>
                                    )}
                                </div>
